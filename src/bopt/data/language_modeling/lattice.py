...
from bopt.data.language_modeling.unigram import viterbi_tokenize
...
def preprocess_language_modeling_with_viterbi_lattices_dataset(data_file: str,
                   cache_dir: str,
                   input_tokenizer: Tokenizer,
                   output_vocab: Integerizer,
                   max_blocks: int = None,
        ...
        for i, line in enumerate(tqdm(textfile)):
            ...
            for packed_chunk in packed_chunks:
                ...
                for chunk in packed_chunk:
                    ...
                    pos += 1
                new_packed_chunks.append(new_packed_chunk)
            _, new_fwd_ms, _, _, new_bwd_ms, _, _, _ = input_tokenizer.encode_packed_batch(new_packed_chunks, max_unit_length, max_block_length, compact=True, verbatim=True)
            fwd_ms *= new_fwd_ms
            bwd_ms *= new_bwd_ms
            fwd_ids = (fwd_ids * fwd_ms + (1-fwd_ms) * input_tokenizer.pad_index).to(torch.long)
            bwd_ids =  (bwd_ids * bwd_ms + (1-bwd_ms) * input_tokenizer.pad_index).to(torch.long)
            ids, mask, pos_ids, lm_ids, lm_mask, lm_pos_ids = input_tokenizer.integerize_packed_chunks(packed_chunks, max_unit_length, max_block_length)
            item_name = os.path.join(cache_dir, f"{i}.pkl")
            binary_mask = torch.cat([mask, lm_mask], 0)
            # causal_mask = input_tokenizer.causal_mask(max_blocks, max_block_length, max_unit_length)
...
def preprocess_language_modeling_with_lattices_output_viterbi_dataset(args, data_file: str,
    ...
    with open(data_file, encoding='utf_8') as textfile:
        for i, line in enumerate(tqdm(textfile)):
            ...
            for packed_chunk in packed_chunks:
                ntokens.append(len(packed_chunk))
            fwd_ids, fwd_ms, lengths, bwd_ids, bwd_ms, bwd_lengths, mmask, emask = input_tokenizer.encode_packed_batch(
                packed_chunks, max_unit_length, max_block_length, compact=True)
            ...
            for packed_chunk in packed_chunks:
                ...
                for chunk in packed_chunk:
                    ...
                    pos += 1
                new_packed_chunks.append(new_packed_chunk)
            _, new_fwd_ms, _, _, new_bwd_ms, _, _, _ = input_tokenizer.encode_packed_batch(new_packed_chunks,
                                                                                           max_unit_length,
                                                                                           max_block_length,
                                                                                           compact=True,
                                                                                           verbatim=True)
            viterbi_fwd_ms = fwd_ms * new_fwd_ms
            viterbi_bwd_ms = bwd_ms * new_bwd_ms
            viterbi_fwd_ids = (fwd_ids * fwd_ms + (1 - fwd_ms) * input_tokenizer.pad_index).to(torch.long)
            viterbi_bwd_ids = (bwd_ids * bwd_ms + (1 - bwd_ms) * input_tokenizer.pad_index).to(torch.long)

            ids, mask, pos_ids, lm_ids, lm_mask, lm_pos_ids = input_tokenizer.integerize_packed_chunks(packed_chunks,
                                                                                                       max_unit_length,
                                                                                                       max_block_length)
            item_name = os.path.join(cache_dir, f"{i}.pkl")
            binary_mask = torch.cat([mask, lm_mask], 0)
