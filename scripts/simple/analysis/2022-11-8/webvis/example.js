...
var checkpoints = [0, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]
...
function reload(){
    d3.json(`../lattices/${ents[ent]}/${datasets[dataset]}/${checkpoints[checkpoint]}/${example}.json`).then(draw_svg);
}
...
d3.select("body").append("br")
d3.select("body").append("text").text(`entropy_reg: ${ents[ent]}`).attr("id", "ent_text")
d3.select("body")
    .append("input")
    .attr("id", "ent_bar")
    .attr("type", "range")
    .attr("min", 0)
    .attr("max", ents.length-1)
    .attr("value", 0)
    .style("width", "100px")
    .on("input", function () {
        clear_all_canvas()
        ent = d3.select("#ent_bar").property("value")
        reload()
        d3.select("body").select("#ent_text").text(`entropy_reg: ${ents[ent]}`)
    })
