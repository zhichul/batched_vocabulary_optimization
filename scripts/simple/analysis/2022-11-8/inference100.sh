#!/usr/bin/env bash
#$ -wd /home/zlu39/jhu/bopt/scripts/simple/exp1-14
#$ -V
#$ -N s3e1-14
#$ -j y -o /export/c01/zlu39/jobs/$JOB_NAME-$JOB_ID.out
#$ -M zlu39@jhu.edu
#$ -m e
#$ -l ram_free=35G,mem_free=35G,gpu=1,hostname=c0*|c1*
#$ -q g.q

source /home/gqin2/scripts/acquire-gpus 1
conda env list
echo "CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"

EXPID="1-16"
mkdir -p ${BLU_ARTIFACTS}/bopt/simple/exp${EXPID}
DATA_PREFIX=${BLU_CORPORA}/simple
ARTIFACT_PREFIX=${BLU_ARTIFACTS}/bopt/simple/exp${EXPID}
SCRIPT_PREFIX=${HOME}/jhu/bopt/scripts/simple/exp${EXPID}

rm ${ARTIFACT_PREFIX}/42/100.0/768/full_lattice_log.train.json
rm ${ARTIFACT_PREFIX}/42/100.0/768/full_lattice_log.valid.json

for SEED in 42
do
for SIZE in 768
do
for ENT in 100.0
do
for CKPT in 0
do
for DAT in train valid
do
CUDA_VISIBLE_DEVICES=1 python3 -O -um bopt.run \
    --seed ${SEED} \
    --train_dataset ${DATA_PREFIX}/se.${DAT}.128.txt \
    --eval_dataset ${DATA_PREFIX}/se.${DAT}.128.txt \
    --input_vocab ${DATA_PREFIX}/substring8-vocab-threshold=None.txt \
    --output_vocab ${DATA_PREFIX}/substring8-vocab-threshold=None.txt \
    --config ${SCRIPT_PREFIX}/config${SIZE}.json \
    --output_dir ${ARTIFACT_PREFIX}/${SEED}/${ENT}/${SIZE}/ \
    --overwrite_output_dir \
    --do_eval \
    --vopt \
    --bias_mode albo \
    --eval_epochs 1 \
    --save_epochs 1 \
    --train_batch_size 1 \
    --gpu_batch_size 4 \
    --continuing_subword_prefix @@ \
    --task language_modeling \
    --entropic ${ENT} \
    --entropy_start 126 \
    --entropy_end 127 \
    --max_blocks 4 \
    --max_block_length 32 \
    --max_unit_length 8 \
    --warmup_epochs 1 \
    --constant_normalization 14.65 \
    --length_normalized_initialization \
    --log_lattice unit marginal log_prob ent lm_marginal \
    --log_lattice_file ${ARTIFACT_PREFIX}/${SEED}/${ENT}/${SIZE}/full_lattice_log.${DAT}.json \
    --log_lattice_key ${CKPT} \
    --marginal_temperature 100.0 \
    --debug_fixed_point

done
done
done
done
done

for SEED in 42
do
for SIZE in 768
do
for ENT in 100.0
do
for CKPT in $(seq 100 100 1000)
do
for DAT in train valid
do
CUDA_VISIBLE_DEVICES=1 python3 -O -um bopt.run \
    --seed ${SEED} \
    --train_dataset ${DATA_PREFIX}/se.${DAT}.128.txt \
    --eval_dataset ${DATA_PREFIX}/se.${DAT}.128.txt \
    --input_vocab ${DATA_PREFIX}/substring8-vocab-threshold=None.txt \
    --output_vocab ${DATA_PREFIX}/substring8-vocab-threshold=None.txt \
    --config ${SCRIPT_PREFIX}/config${SIZE}.json \
    --output_dir ${ARTIFACT_PREFIX}/${SEED}/${ENT}/${SIZE}/ \
    --overwrite_output_dir \
    --do_eval \
    --vopt \
    --bias_mode albo \
    --eval_epochs 1 \
    --save_epochs 1 \
    --train_batch_size 1 \
    --gpu_batch_size 4 \
    --continuing_subword_prefix @@ \
    --task language_modeling \
    --entropic ${ENT} \
    --entropy_start 126 \
    --entropy_end 127 \
    --max_blocks 4 \
    --max_block_length 32 \
    --max_unit_length 8 \
    --warmup_epochs 1 \
    --constant_normalization 14.65 \
    --length_normalized_initialization \
    --model_name ${ARTIFACT_PREFIX}/${SEED}/${ENT}/${SIZE}/checkpoint-${CKPT} \
    --log_lattice unit marginal log_prob ent lm_marginal \
    --log_lattice_file ${ARTIFACT_PREFIX}/${SEED}/${ENT}/${SIZE}/full_lattice_log.${DAT}.json \
    --log_lattice_key ${CKPT} \
    --marginal_temperature 100.0 \
    --debug_fixed_point

done
done
done
done
done