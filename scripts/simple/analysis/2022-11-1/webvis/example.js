...
d3.json("../lattices/train/300/0.json").then(function (d){draw(canvas, d, "marginal");draw(canvas1, d, "log_prob")});
function draw(canvas, data, field) {
    ...
    data.forEach(function (node, i) {
        let edge_x = d => (i + d["edge"]["length"]-1) * 2 * unit * 2 -(d["edge"]["length"]-1) * unit * 2  + unit * 3
...
svg.call(zoom);
var checkpoints = [300, 600, 900, 1200, 1500, 1800, 2100, 2400];
var checkpoint = 0;
var example = 0;
d3.select("body").append("br")
d3.select("body").append("text").text(`checkpoint: ${300}`).attr("id", "ckpt_text")
d3.select("body")
    .append("input")
    .attr("id", "ckpt_bar")
    .attr("type", "range")
    ...
    .on("input", function () {
        ...
        d3.json(`../lattices/train/${checkpoints[checkpoint]}/${example}.json`).then(function (d){draw(canvas, d, "marginal");draw(canvas1, d, "log_prob")});
