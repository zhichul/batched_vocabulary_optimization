...
CUDA_VISIBLE_DEVICES=0 python3 -O -um bopt.run \
    ...
    --weights_file /export/a01/artifacts/bopt/syn3/exp${load}/42/2/4/96/${L1}/late_ent/checkpoint-${CK}/learned_vocab.txt  \
    --output_vocab ${DATA_PREFIX}/output_vocab.txt \
    --config ${SCRIPT_PREFIX}/config${LAYER}-${HEAD}-${SIZE}.json \
    --output_dir ${ARTIFACT_PREFIX}/${SEED}/${LAYER}/${HEAD}/${SIZE}/${L1}/${load}/${CK}0 \
