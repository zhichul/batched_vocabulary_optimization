...
CUDA_VISIBLE_DEVICES=1 python3 -O -um bopt.run \
    ...
    --input_vocab ${DATA_PREFIX}/unigram-vocab-${UNI}.txt \
    --weights_file ${DATA_PREFIX}/unigram-weights-${UNI}.txt \
    --output_dir ${ARTIFACT_PREFIX}/${SEED}/${L1}/${SIZE}/ \
    --overwrite_output_dir \
    --config ${SCRIPT_PREFIX}/config${SIZE}.json \
    --task language_modeling > ${ARTIFACT_PREFIX}/${SEED}/${L1}/${SIZE}/unigram-${UNI}-4929.seg.txt
done
done
done
done
