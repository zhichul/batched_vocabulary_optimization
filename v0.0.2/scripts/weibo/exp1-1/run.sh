#!/usr/bin/env bash
EXPID="1-1"
mkdir -p ${BLU_ARTIFACTS}/boptv2/weibo/exp${EXPID}
DATA_PREFIX=${BLU_CORPORA}/weibo/
ARTIFACT_PREFIX=${BLU_ARTIFACTS}/boptv2/weibo/exp${EXPID}
SCRIPT_PREFIX=${HOME}/jhu/bopt/v0.0.2/scripts/weibo/exp${EXPID}
...
CUDA_VISIBLE_DEVICES=1 CUDA_LAUNCH_BLOCKING=1 python3 -O -um bopt.train \
    ...
    --config ${CONFIG_NAME} \
    \
    --input_vocab ${DATA_PREFIX}/substring-vocab-max_length\=10-min_count\=10.50k.txt \
    ...
    --train_steps 1200 \
    --patience 10 \
    --lr_adjustment_window_size 669054 \
    --reduce_factor 0.25 \
    \
    --eval_steps 60 \
    \
