...
OUTPUT_PREFIX=${ARTIFACT_PREFIX}/${SEED}/${SIZE}/${L1}/${DATA}/${CKPT}

OUTPUT_DIR=${OUTPUT_PREFIX}/token-pos
CUDA_VISIBLE_DEVICES=0 CUDA_LAUNCH_BLOCKING=1 python3 -O -um bopt.train \
    ...
    --config  ${CONFIG_NAME} \
    \
    --input_vocab ${TOKENIZER_PREFIX}/learned_vocab.txt \
    --input_tokenizer_weights ${TOKENIZER_PREFIX}/learned_vocab.txt \
...
OUTPUT_DIR=${OUTPUT_PREFIX}/char-pos
CUDA_VISIBLE_DEVICES=0 CUDA_LAUNCH_BLOCKING=1 python3 -O -um bopt.train \
    --output_directory ${OUTPUT_DIR} \
    --overwrite_output_directory \
    \
