function lattice_hyperopt () {
  ...
  do
    ...
    python3 -m experiments.scripts.best_dev \
      hyperopt.${DATA}.tmp ${METRIC} ${ORDER} \
      step \
      dev_accuracy \
      test_accuracy \
      train_accuracy \
    ...
    do
      python3 -m experiments.scripts.json_join \
        --path "${ARTIFACT_PREFIX}/{0}/${SIZE}/{1}/${DATA}/${CHECKPOINT}/${INPUT_NAME}.1best.tokenizations.f1.json" \
    ...
    cat hyperopt.${DATA}.results.tmp >> ${RESULTS_FILE}
...
function lattice_full_attention_hyperopt () {
  ...
  do
    ...
    do
      for SEED in 42 44 46 48 50 # 52 54 56 58 60
      do
        OUTPUT_DIR=${ARTIFACT_PREFIX}/${SEED}/${SIZE}/${DATA}
        python3 -m experiments.scripts.best_dev \
          ${OUTPUT_DIR}/log.json ${METRIC} ${ORDER} \
          step \
          dev_accuracy \
...
function unigram_1best_hyperopt () {
  # this function optimizes the hyperparameters other than training DATA,
  # and the step (always takes the final iteration, i.e. after entropy regularization)
  # then merges the tokenization metrics in from the final iteration, and saves
  # to hyperopt-results.json
...
function unigram_gold_hyperopt () {
  # this function optimizes the hyperparameters other than training DATA,
  # and the step (always takes the final iteration, i.e. after entropy regularization)
  # then merges the tokenization metrics in from the final iteration, and saves
  # to hyperopt-results.json
