...
OUTPUT_DIR=${ARTIFACT_PREFIX}/${SEED}/${SIZE}/${L1}/${DATA}/${AHEAD}/${TRAJ}/${BIAS}/${WARMUP}/combined/${LR}/${MOMENTUM}/${TRAINRANDOM}/${EVALRANDOM}/safe
...
CUDA_VISIBLE_DEVICES=0 CUDA_LAUNCH_BLOCKING=1 python3 -O -um bopt.train_bilevel \
    ...
    --inner_threshold 1e-30 \
    --bilevel_optimization_scheme reversible-learning \
    --train_batch_size_inner 64 \
    --gpu_batch_size_inner 64 \
    --random_restarts ${TRAINRANDOM} \
    --eval_random_restarts ${EVALRANDOM} \
    --momentum_coefficient ${MOMENTUM} \
    --eval_train_steps ${AHEAD} \
    --log_trajectory \
    --log_trajectory_steps 5 \
