...
LOAD_ARTIFACT_PREFIX=${BLU_ARTIFACTS2}/boptv2/syn4_small/exp21-9-reversible-3

for SEED in 42 44 46 48 50
do
for SIZE in 768
do
for L1 in 0.00 # 0.01 0.1 1.0
do
...
TRAIN_NAME=train.inner.csv
DEV_NAME=dev.csv
TEST_NAME=test.csv
CONFIG_NAME=${SCRIPT_PREFIX}/config${SIZE}.json
LOAD_DIR=${LOAD_ARTIFACT_PREFIX}/42/768/0.00/500/100/1/mult_then_renorm/0/combined/6.25e-3/0.1/5/3/safe/checkpoint-final

CUDA_VISIBLE_DEVICES=0 CUDA_LAUNCH_BLOCKING=1 python3 -O -um bopt.train_reversible \
    ...
    --split_on_space \
    \
    --task_model_learning_rate 6.25e-3 \
    --input_tokenizer_learning_rate ${LR} \
    --train_batch_size 64 \
    --train_steps 600 \
    --patience 10 \
    --lr_adjustment_window_size 64 \
    --reduce_factor 0.25 \
    \
    --eval_steps 30 \
    \
