#!/usr/bin/env bash
EXPID="24-1"
mkdir -p ${BLU_ARTIFACTS}/boptv2/syn4_small/exp${EXPID}/debug
...
OUTPUT_DIR=${ARTIFACT_PREFIX}/debug/${SEED}/${SIZE}/${L1}/${DATA}
...
CUDA_VISIBLE_DEVICES=0 CUDA_LAUNCH_BLOCKING=1 python3 -O -um bopt.train \
    ...
    \
    --bias_mode albo \
    --config  ${ARTIFACT_PREFIX}/${SEED}/${SIZE}/${L1}/${DATA}/checkpoint-early-stopping/config.json \
    --pretrained_model ${ARTIFACT_PREFIX}/${SEED}/${SIZE}/${L1}/${DATA}/checkpoint-early-stopping/ \
    \
    --input_vocab ${ARTIFACT_PREFIX}/${SEED}/${SIZE}/${L1}/${DATA}/checkpoint-early-stopping/learned_vocab.txt \
    --input_tokenizer_weights ${ARTIFACT_PREFIX}/${SEED}/${SIZE}/${L1}/${DATA}/checkpoint-early-stopping/learned_vocab.txt \
    ...
    \
    --annealing 10.0 \
    --annealing_start_steps 0 \
    --annealing_end_steps 150 \
    --L1 ${L1} \
    \
    --gpu_batch_size 128 \
    --device "cuda"
