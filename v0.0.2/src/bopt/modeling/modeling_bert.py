...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if not self.no_pos and self.position_embedding_type == "absolute":
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        override_size = config.override_output_vocab_size
        self.decoder = nn.Linear(config.hidden_size, config.vocab_size if override_size is None else override_size, bias=False)

        self.bias = nn.Parameter(torch.zeros(config.vocab_size if override_size is None else override_size))
