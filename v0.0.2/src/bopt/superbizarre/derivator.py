...
class Derivator:
    ...
    def __init__(self, f_pfxes='pfxes.txt', f_sfxes='sfxes.txt', f_stems='stems.txt', n_stems=None):
        ...
        self.sfxes = list()
        self.stems = set()
        f_pfxes = os.path.join(os.path.dirname(__file__), f_pfxes)
        f_sfxes = os.path.join(os.path.dirname(__file__), f_sfxes)
        f_stems = os.path.join(os.path.dirname(__file__), f_stems)
        # Load prefixes
...
if __name__ == "__main__":
    ...
    for line in sys.stdin:
        ...
        ends = [sum(len(piece) for piece in pieces[:i+1]) for i in range(len(pieces))]
        # print(repr("".join(pieces)), repr(word), repr(derivation), "".join(pieces)==word,  file=sys.stderr)
        if "".join(pieces) == word:
            display(word, [list(zip(pieces, ends))], [1.0])